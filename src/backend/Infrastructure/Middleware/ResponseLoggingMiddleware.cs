...
namespace CodeMatrix.Mepd.Infrastructure.Middleware;
...
public class ResponseLoggingMiddleware : IMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext httpContext, RequestDelegate next)
    {
        ...
        string responseBody;
        if (httpContext.Request.Path.ToString().Contains("tokens", StringComparison.OrdinalIgnoreCase))
        {
        ...
        if (!string.IsNullOrWhiteSpace(userId) && userId != Guid.Empty.ToString()) LogContext.PushProperty("UserId", userId);
