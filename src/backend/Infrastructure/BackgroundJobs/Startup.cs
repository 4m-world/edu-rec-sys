...
namespace CodeMatrix.Mepd.Infrastructure.BackgroundJobs;
...
internal static class Startup
{
    ...
    {
        services.AddHangfireServer(options => config.GetSection("HangfireSettings:Server").Get<BackgroundJobServerOptions>());
        ...
        services.AddSingleton<JobActivator, MepdJobActivator>();
        ...
        services.AddHangfire((provider, hangfireConfig) => hangfireConfig
            ...
            .UseFilter(new MepdJobFilter(provider))
    ...
    private static IGlobalConfiguration UseDatabase(this IGlobalConfiguration hangfireConfig, string provider, string connection, IConfiguration config)
    {
        return provider.ToLowerInvariant() switch
        {
            DbProviderKeys.SqlServer =>
                hangfireConfig
                    .UseSqlServerStorage(connection, config.GetSection("HangfireSettings:Storage:Options").Get<SqlServerStorageOptions>()),
            _ => throw new Exception($"Hangfire Storage Provider {provider} is not supported.")
        };
    }
